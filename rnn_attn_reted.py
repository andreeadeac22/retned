...
def train_valid_model(filename, which_train, which_evaluate, model, train_data, valid_data, optimizer, criterion):
	...
	plot_loss(filename = "results/" + filename + "_losses", train_losses=train_losses, valid_losses=valid_losses, num_epochs=epoch)
...
def main():
	...
	ew_test_data = torch.cat((test_data, sim_test_data), dim=1)

    ############################### TSNE #################################

	#tsne_test_sample = enc_test_vect[0]
	num_tsne_train_data = 1000
